                    ...
                    @foreach (ProjectStatus::all() as $statusId => $status)
                        <div class="col-lg-6 col-md-4 col-xs-6">
                            @include('view-components.dashboard-panel', [
                                ...
                                'number' => array_key_exists($statusId, $projectStatusStats)
                                    ? $projectStatusStats[$statusId]
                                    : 0,
            ...
            <div class="col-lg-7">
                ...
                <div class="panel panel-default table-responsive hidden-xs">
                    <table class="table table-condensed table-bordered">
                        <tr>
                            <td class="col-xs-2 text-center">{{ __('dashboard.yearly_earnings') }} ({{ $queriedYear }})
                            </td>
                            <td class="col-xs-2 text-center">{{ __('dashboard.finished_projects_count') }}
                                ({{ $queriedYear }})</td>
                ...
                <legend style="border-bottom: none" class="text-center">{{ __('dashboard.upcoming_subscriptions_expiry') }}
                </legend>
                    ...
                    <table class="table table-condensed">
                        ...
                        @forelse(AdminDashboard::upcomingSubscriptionDueDatesList() as $subscription)
                            <tr>
                                ...
                                <td>{{ link_to_route('customers.subscriptions', $subscription->customer->name, [$subscription->customer]) }}
                                </td>
                        ...
                        @empty
                            <tr>
                                <td colspan="4">{{ __('dashboard.no_upcoming_subscriptions_expiry') }}</td>
                            </tr>
                        @endforelse
                        <tr>
                            <th colspan="4" class="text-center">
                                {{ link_to_route('subscriptions.index', __('subscription.view_all')) }}</th>
                        </tr>
                    </table>
                </div>
            </div>
            ...
            <div class="col-md-4 col-md-offset-3">
                <div class="panel panel-default">
                    <div class="panel-heading">
                        <h3 class="panel-title">{{ __('user.current_jobs') }}</h3>
                    </div>
                    <table class="table table-condensed">
                        <tbody>
                            @php
